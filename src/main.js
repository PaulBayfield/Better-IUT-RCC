...
import { addButtons, cleanCards, createBilanCard, generateHtml, orderCards, fetchAllSortedGrades, recreateTable, updateMenu, addSearchBar, addSaveButton, addResetButton, applyDarkTheme, addAvisNotification, addMessageForNotifications } from "./functions";
...
(async () => {
    if (window.location.hostname === "cas.univ-reims.fr" && window.location.pathname === "/cas/login") {
        console.info("[Better IUT RCC] Page de connexion CAS détectée !");

        chrome.storage.sync.get("enableNotifications", async function(data) {
            if (!data.enableNotifications) {
                console.info("[Better IUT RCC] Notifications désactivées ! Identifiants non sauvegardés.");

                return;
            } else {
                console.info("[Better IUT RCC] Sauvegarde des identifiants...");

                const form = document.querySelector("form");
                const submit = document.querySelector("button[type='submit']");
        
                if (form && submit) {
                    console.info("[Better IUT RCC] Formulaire de connexion trouvé !");
        
                    const username = document.querySelector("input[id='username']");
                    const password = document.querySelector("input[id='password']");
        
                    if (!username || !password) {
                        console.error("[Better IUT RCC] Identifiants non trouvés !");
                        return;
                    }
        
                    username.addEventListener('change', (e) => {
                        browser.storage.sync.set({secret_0: e.target.value});
        
                        console.info("[Better IUT RCC] Identifiant 0 trouvé !");
                    });
        
                    password.addEventListener('change', (e) => {
                        browser.storage.sync.set({secret_1: e.target.value});
        
                        console.info("[Better IUT RCC] Identifiant 1 trouvé !");
                    });
                }
            }
        });
        ...
        if (window.location.pathname === "/fr/tableau-de-bord") {
            ...
            browser.storage.sync.get('notesAlreadyKnow').then((result) => {
                ...
                browser.storage.sync.get('hideNotesNotification').then((result) => {
                    if (result.hideNotesNotification === undefined || result.hideNotesNotification === false) {
                        addMessageForNotifications().scrollIntoView({ behavior: "smooth" });
                    }
                });
            });
        } 
